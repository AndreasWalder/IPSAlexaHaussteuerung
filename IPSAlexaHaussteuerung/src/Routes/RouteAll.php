...
final class RouteAll{
    public function handle(array $payload,array $cfg):array{
        ...
        switch($route){
            ...
            case 'main_launch':return $this->handleMain($payload,$cfg);
        }
        $dynamic=$this->handleRendererDomainRoute($route,$payload,$cfg);
        if($dynamic!==null)return $dynamic;
        return $this->handleMain($payload,$cfg);
    }
    private function handleMain(array $payload,array $cfg):array{$payload=$this->withRendererCfg($payload,$cfg);$custom=(new RenderMain())->process($payload,$cfg);if(is_array($custom))return $custom;return $this->baseResponse($payload,$cfg,'hv-main','Main','main');}
    private function handleHeizung(array $payload,array $cfg):array{$payload=$this->withRendererCfg($payload,$cfg);$custom=(new RenderHeizung())->process($payload,$cfg);if(is_array($custom))return $custom;return $this->baseResponse($payload,$cfg,'hv-heizung','Heizung','heizung');}
    private function handleJalousie(array $payload,array $cfg):array{$payload=$this->withRendererCfg($payload,$cfg);$custom=(new RenderJalousie())->process($payload,$cfg);if(is_array($custom))return $custom;return $this->baseResponse($payload,$cfg,'hv-jalousie','Jalousie','jalousie');}
    private function handleLicht(array $payload,array $cfg):array{$payload=$this->withRendererCfg($payload,$cfg);$custom=(new RenderLicht())->process($payload,$cfg);if(is_array($custom))return $custom;return $this->baseResponse($payload,$cfg,'hv-licht','Licht','licht');}
    private function handleLueftung(array $payload,array $cfg):array{$payload=$this->withRendererCfg($payload,$cfg);$custom=(new RenderLueftung())->process($payload,$cfg);if(is_array($custom))return $custom;return $this->baseResponse($payload,$cfg,'hv-lueftung','Lueftung','lueftung');}
    private function handleGeraete(array $payload,array $cfg):array{$payload=$this->withRendererCfg($payload,$cfg);$custom=(new RenderGeraete())->process($payload,$cfg);if(is_array($custom))return $custom;return $this->baseResponse($payload,$cfg,'hv-geraete','Geraete','geraete');}
    private function handleBewaesserung(array $payload,array $cfg):array{$payload=$this->withRendererCfg($payload,$cfg);$custom=(new RenderBewaesserung())->process($payload,$cfg);if(is_array($custom))return $custom;return $this->baseResponse($payload,$cfg,'hv-bewaesserung','Bewasserung','bewaesserung');}
    ...
    private function resolveRendererForDomain(array $domain){$roomDomain=strtolower((string)($domain['roomDomain']??''));$aplDoc=strtolower((string)($domain['aplDoc']??''));if($roomDomain==='sprinkler'||strpos($aplDoc,'bewaesserung')!==false)return new RenderBewaesserung();return new RenderGeraete();}
